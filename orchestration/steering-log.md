...
- 루프 완료: 게이트 G1-G8 통과 · 3-리뷰어 문서 docs/gate-*.md 머지 · em-dash 텍스트레이어 0건 · U1 EN 7화면 780px 재생성 완료(flows-v2-en/)
- 잔여(fix-next): §7 CTO 재심 문서화 · 7화면 §14 재판정(TONE) · 프로덕션 카피덱 JSON · 10-로케일 데모 확장 · 서비스명 {{BRAND}} 확정 데드라인
- S5 (09-05): 기존 자료(hot14 아카이브 291건 + review-v2)에 **추가 조사 자료**를 더해 밝은 물성 방향에 맞는 레퍼런스를 보강하라 — 기존 판정은 다크 시대 기준이므로 밝은 방향 재판정 필요. 먹선 일러스트(ink-ill-04/08/10) + 상징 로고(b6cde5b) 스타일을 작업에 반영.
- S6 (09-05): typography-demo 확인·반영(multilang.css 리뷰 페이지 링크) + AI-slop 감사(10-로케일 데모·리뷰 페이지 통과) + U1.6 entry 이미지-온리 결함 → edit_screens로 폼 UI 완성(신규 screen a931a656).
- 루프 R4-R5 완료: Swiss 고도화 세트(6화면 780px) · Neo-Brutalist 10변형 · 3대 연구 문서(페르소나 10·퍼널·메타인지) · T-01~T-12 티켓 · 이슈 #2~#6
- 결정 대기: (1) Swiss 확정 여부 (2) Q1 사이트+앱 vs 웹 전용 (3) Q5 로케일 전략 (4) {{BRAND}} 확정
