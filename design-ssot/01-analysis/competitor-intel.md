...
- 미해결: 포스텔러 URL(외부 재조사로 forceteller.com 확인됨, 본 문서 §1 보강), 국내 경쟁사 스크린샷 수준의 시각 자료는 파일에 없음. 혼빛은 미러로 완전 검증 가능.

---

## 6. 2026-09-05 확대 정찰 — K-pop 사주 니치 신규 5곳 (캡처 보유)

브라우저 실측 캡처: `assets/competitors/kpop-{idolsaju,sajufromseoul,jayinseoul,sajumuse}-0*.png` (2026-09-05)

| 서비스 | 포지셔닝 | 우리와의 관계 |
|---|---|---|
| idolsaju.com | 아이돌 200+ 호환 계산기 + AI 운세(타로·손금 결합), 한국어 기본 | 계산기 유틸리티 포지션. 깊이 없음 |
| sajufromseoul.com | 60+ 아이돌 Day Master 표 공개, 무료 차트 → AI 리더, 블로그 SEO 공세(2026-03~) | Day Master 표 공개 전략 선점. 물상론 깊이는 없음 |
| **jayinseoul.com** | 출처 기반 셀럽 차트 아카이브, "The Oak/The River" 오브제 명명, "Pattern not prediction", Day Master 계산기, 2026-08 활발 갱신 | **최근접 위협.** 근거 원칙·오브제 명명·톤까지 겹침. 차별축: 남촌 단일 학파 깊이 + 고정 시각 문법 + 공유 카드 상품화 + 다크 프리미엄 무드(상대는 라이트 에디토리얼) |
| sajumuse.com | 기사(certified master) 포지셔닝, 리포트 $29·커플 $55, BTS Day Master 분석 콘텐츠, 138+ 리뷰 | 가격 상단 참고. 인간 마스터 의존이 확장 한계 |
| sajuatelier.com | 무료 호환 계산기 + 저널 | 하단 참고 |

**전략 판정 갱신**: "셀럽 궁합 계산기" 자체는 만원. 차별화 4축(물상론 깊이·근거 제품화·시각 문법 자산화·카드 미학)이 겹치는 자리는 여전히 비어 있고, 이는 02-direction 확정 방향과 정확히 일치한다. 출시 가속(가을 피크 전) 논거가 강해짐.

추가 발견: palja(K-Pop Saju Compatibility, railway 앱) 등 소형 플레이어도 확인. 포스텔러 URL은 외부 재조사로 forceteller.com 확정(본 문서 §1의 "URL 미기재" 해소).
